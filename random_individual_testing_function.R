#Code for the random individual testing function
...
spread <- function( s_nodes, v_nodes, e_nodes_info, current_infectious, direct_VE,incperiod_shape, incperiod_rate,susc_list=contact_list,beta_scalar=1){
  ...
  newinfected <- funique(infectees_susc)
  # infect vaccinated
  if(sum(v_nodes)>0){
    infectees_susc <- c()
...
simulate_contact_network <- function(first_infected,individual_recruitment_times=F,end_time=31,start_day=0,from_source=0,cluster_flag=0,allocation_ratio=0.5,
  ...
  # identify contacts of index cases
  ...
  # identify contacts of contacts of index case
  ...
  for(time_step in 1:sim_time){
    ...
    if ((nrow(e_nodes_info)>0)||(nrow(i_nodes_info)>0)) {
      ...
      ## identifying the population that can be used in the testing scheme
      ...
      #Those that have been quarantined for 14 days are relased from quarantine and no longer eligible for testing
      for (i in 1:length(g_name))
        if (q_nodes[i] == 1){
      ...
      if(length(infected_persons) > 0){
        positive[time_step,1:length(infected_persons)] <- infected_persons  
      } 
    }
    # Metric of the number of days new infections cross a threshold
