rm(list=ls())
setwd('/Users/bethanyheath/OneDrive/bethany/PhD/Pan_Code_3')
...
covid_spread_wrapper <- function(i_nodes_info,s_nodes,v_nodes,e_nodes_info,isolation_individuals, direct_VE){
  ...
  if(length(isolation_individuals)>0){
    e_nodes_info <- spread(s_nodes,v_nodes,e_nodes_info,isolation_individuals,direct_VE,incperiod_shape,incperiod_rate,susc_list=household_list,beta_scalar=1)
  }
  return(e_nodes_info)
...
#### Additional code ####
my_log6 <- file("my_log6.txt")
nIter <- 100
e_order <- list()
#profvis({
for(iter in 1:nIter){
  ## select random person to start
  first_infected <- sample(g_name, 15)
  inf_period <- rgamma(length(first_infected),shape=infperiod_shape,rate=infperiod_rate)
  netwk <- simulate_contact_network(first_infected,start_day=iter,from_source=0,cluster_flag=0,individual_recruitment_times=T,spread_wrapper=covid_spread_wrapper, prob_false_neg = 0)
  results_list[[iter]] <- netwk[[1]]
  cluster_size[iter] <- netwk[[2]]
  #recruit_times[iter] <- netwk[[3]]
  e_order[[iter]] <- netwk[[8]][!duplicated(netwk[[8]])]
  print(netwk[11])
  sink(my_log6, append=TRUE, type="output")
}
