...
simulate_contact_network <- function(first_infected,individual_recruitment_times=F,end_time=31,start_day=0,from_source=0,cluster_flag=0,allocation_ratio=0.5,
                                     ...
                                     tests, delay_prob, non_compliance_prob){
  ...
  #contacts <-funique(c(contacts_1, contacts_2, contacts_3, contacts_4, contacts_5, contacts_6, contacts_7, contacts_8, contacts_9, contacts_10))
  ...
  #contacts_of_contacts <-funique(c(contacts_of_contacts_1, contacts_of_contacts_2, contacts_of_contacts_3, contacts_of_contacts_4, contacts_of_contacts_5, contacts_of_contacts_6, contacts_of_contacts_7, contacts_of_contacts_8, contacts_of_contacts_9, contacts_of_contacts_10))
  ...
  for(time_step in 1:sim_time){
    ...
    if ((nrow(e_nodes_info)>0)||(nrow(i_nodes_info)>0)) {
      ...
      individuals_wrong <- g_name[f_nodes==1]
      if (length(individuals_wrong)>0){
        infected_individuals <- infected_individual[infected_indivi %ni% individuals_wrong]
      }else {
        infected_individuals <-infected_individual
      }
      
      if (length(infected_individuals) > tests) {
        potential_positive <- sample(infected_individuals, tests, replace=F)
      } else {
        potential_positive <- infected_individuals
      }
      probaility_chose <-c()
      prob_choose <- c()
      for (i in 1:length(potential_positive))
        if (runif(1, 0, 1) < delay_prob){
          prob_choose[i] <- potential_positive[i]
        }
    
      testing_pos <- potential_positive[potential_positive %ni% prob_choose]
      
     positive_testing_wrong <-c()
     for (i in 1:length(testing_pos))
       if (runif(1, 0, 1) < prob_false_neg){
         positive_testing_wrong[i] <- testing_pos[i]
       }
      
     accurate_positive <- testing_pos[testing_pos %ni% positive_testing_wrong]
      
      names_of <- sample(potential_positive, round((1-prob_false_neg)*length(potential_positive)), replace = F)
